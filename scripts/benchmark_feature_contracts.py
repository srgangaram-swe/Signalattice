...
from quant_platform.features.contracts import build_contract_features, default_contracts
...
def main() -> None:
    n_contracts = len(default_contracts())
    cases = []
    for tickers, dates in PANEL_SIZES:
        frame = _panel(tickers, dates, seed=7)
        tracemalloc.start()
        features, seconds = _timed(lambda frame=frame: build_contract_features(frame))
