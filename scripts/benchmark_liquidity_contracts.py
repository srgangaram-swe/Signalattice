"""Deterministic local benchmark for the SF-S2-MR3 liquidity feature contracts.

Reports wall time and peak memory for the volume, liquidity, spread, and impact
contracts across representative panel sizes. These estimators are vectorised
(rolling reductions), so they scale like the conventional MR1 set. Fully offline
and deterministic; laptop-scale numbers, not a distributed benchmark.
...
from quant_platform.features.contracts import build_contract_features, liquidity_contracts
...
def main() -> None:
    ...
    result = {
        ...
        "limitations": [
            ...
            "excludes turnover (needs a shares_outstanding column)",
        ],
    }
